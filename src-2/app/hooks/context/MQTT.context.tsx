...
import { WifiContext, WifiContextType } from "./Wifi.context";
...
export const MQTTContextProvider: React.FC<MQTTContextProviderProps> = ({ children }) => {
    ...
    const {isWifiConnected} = useContext(WifiContext) as WifiContextType;
    const { userInfo } = useContext(AuthContext) as AuthContextProps;
    ...
    React.useEffect(() => {
        if (isWifiConnected && userInfo !== null) {
            handleConnectMQTT();
        } else {
            handleDisconnectMQTT();
        }
    }, [isWifiConnected, userInfo])
