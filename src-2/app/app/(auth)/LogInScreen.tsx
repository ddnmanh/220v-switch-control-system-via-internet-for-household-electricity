...
const LogInScreen = () => {
    ...
    const [passwordInput, setPasswordInput] = React.useState({ value: '123456@Abc', errorMessage: '' });
    ...
    const handleLogIn = async () => {
        ...
        try {
            ...
            setTimeLiveAccessToken && setTimeLiveAccessToken(response.data.access_token.expired_in*1000);
            ...
            if (response.status === 400) {
                response.invalid_field.forEach((invalidField:ResponseInvalidFieldDTO) => {
                    if (invalidField.field === 'username') {
                        setUsernameInput({
                            ...usernameInput,
                            errorMessage: invalidField.message,
                        });
                    }
                    if (invalidField.field === 'password') {
                        setPasswordInput({
                            ...passwordInput,
                            errorMessage: invalidField.message,
                        });
