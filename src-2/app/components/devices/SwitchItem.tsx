...
const SwitchItemDevice: React.FC<SwitchItemDeviceProps> = ({ device }) => {
    ...
    */
    ...
    React.useEffect(() => {
        if (isMQTTConnected && device?.topic_receive) subscribeTopicMQTT(device?.topic_receive);
        if (isMQTTConnected && device?.topic_send) handleControllToEquipment("STATUS");
    }, [isMQTTConnected, subscribeTopicMQTT])

    React.useEffect(() => {
        const handleMessage = (message: Message) => {
            if (message.destinationName === device?.topic_receive) {
                try {
                    // console.log(
