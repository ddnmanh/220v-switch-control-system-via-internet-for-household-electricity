...
const Device = () => {
    ...
    const handleControllToEquipment = (type: string = "CONTROLL") => {
        ...
        publishToTopicMQTT(
            thisOwnDevice?.topic_send ? thisOwnDevice?.topic_send : "",
            JSON.stringify({
                type: type,
                id: strTimestamp,
    ...
    React.useEffect(() => {
        const handleMessage = (message: Message) => {
            if (message.destinationName === thisOwnDevice?.topic_receive) {
                try {
        ...
        if (newNameDevice !== thisOwnDevice?.name) {
            try {
                const response = await OwnDeviceFetch.update({
                    id: thisOwnDevice?.id,
                    name: newNameDevice,
                    description: thisOwnDevice?.description
                });
