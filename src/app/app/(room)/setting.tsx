...
const RoomSettingScreen = () => {
    ...
    const [newRoomDesc, setNewRoomDesc] = React.useState(roomDataSelected?.description || '');
    ...
    React.useEffect(() => {
        if (roomDataSelected && Object.keys(roomDataSelected).length > 0 && JSON.stringify(roomDataSelected) !== JSON.stringify(thisRoomData)) {
            ...
            setNewRoomDesc(roomDataSelected?.description);
        }
    }, [roomDataSelected]);


    React.useEffect(() => {
        if (newRoomName !== thisRoomData?.name || newRoomDesc !== thisRoomData?.description) {
    ...
    const handleUpdateRoomInfo = async () => {
        ...
        try {
            let response:ResponseDTO = await RoomFetch.update({
                room_id: thisRoomData?.id,
                name: newRoomName,
            ...
            if (response.status === 200 || response.status === 201) {
                handleUpdateDataRoom({...thisRoomData, name: newRoomName, description: newRoomDesc} as RoomWithRelationINF);
            }
