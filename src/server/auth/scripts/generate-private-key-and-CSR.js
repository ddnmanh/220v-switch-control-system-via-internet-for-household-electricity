...
// Lệnh OpenSSL
const command = `openssl req -new -nodes -newkey rsa:2048 -keyout ${path.join(mtlsDir, 'authService.key')} -out ${path.join(mtlsDir, 'authService.csr')} -subj "/C=${country}/ST=${state}/L=${city}/O=${organization}/CN=${domain}"`;
