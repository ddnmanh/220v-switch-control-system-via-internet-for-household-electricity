...
// Lệnh OpenSSL
const command = `openssl req -new -nodes -newkey rsa:2048 -keyout ${path.join(mtlsDir, 'houseService.key')} -out ${path.join(mtlsDir, 'houseService.csr')} -subj "/C=${country}/ST=${state}/L=${city}/O=${organization}/CN=${domain}"`;
